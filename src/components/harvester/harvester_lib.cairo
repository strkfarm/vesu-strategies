use strkfarm_vesu::interfaces::IEkuboDistributor::{
  IEkuboDistributor,
  IEkuboDistributorDispatcher, IEkuboDistributorDispatcherTrait,
  Claim
};
use strkfarm_vesu::components::swap::{
  AvnuMultiRouteSwap,
  AvnuMultiRouteSwapImpl
};
use strkfarm_vesu::components::harvester::interface::{
  IClaimTrait,
  ClaimResult
};
...
> of HarvestConfigTrait<HarvestConfig, TContractState, TSettings1, TSettings2> {
  fn simple_harvest(
    ...
    depositAmount = check_and_swap_harvest(
      claimResult.token, 
      baseTokenAddress, 
      depositAmount, 
      swapInfo,
