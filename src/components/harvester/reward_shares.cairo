// Since starknet rewards are distributed every 2 weeks and there is deterministic way to calculate the exact rewards,
// this module computes unique shares of user for each round, computing their total new shares of underlying contract

use starknet::{ContractAddress};

#[derive(Drop, Copy, Serde, starknet::Store, starknet::Event)]
pub struct RewardsInfo {
    pub amount: u256, // (e.g. STRK for Auto STRK, USDC for Auto USDC)
    pub shares: u256, // shares of underlying contract (e.g. frmzSTRK)
    pub total_round_points: u256, // total points of round which is compared with user shares round to compute user shares
    pub block_number: u64, // When rewards were harvested
}

#[derive(Drop, Copy, Serde, starknet::Store, starknet::Eve)]
pub struct UserRewardsInfo {
    pub pending_round_points: u256, // pending points of user in current round
    pub shares_owned: u256, // total shares of user in underlying contract used to compute harvest shares
...
pub trait IRewardShare<TState> {
    ...
    fn get_total_unminted_shares(self: @TState) -> u256;
    /// @dev just a one time setters for upgrade
    // fn set_last_updated_block(ref self: TState, block_number: u64);
    // fn set_user_rewards_info(ref self: TState, user: ContractAddress, info: UserRewardsInfo, total_shares: u256);
    fn get_additional_shares(self: @TState, user: ContractAddress,) -> (u256, u64, u256);
}
...
pub mod RewardShareComponent {
    ...
    /// Terminology
    /// 1. shares: underlying shares of contract which is minted to represent user's share in contract (e.g. frmzSTRK)
    /// 2. harvest_shares: new shares of underlying asset already distributed to user denominated in underlying shares (e.g. frmzSTRK)
    ...
    pub struct Storage {
        ...
        rewards: LegacyMap<u32, RewardsInfo>, // index => RewardsInfo
        n_rewards: u32,
        // todo add checks to limit how much this can increase between blocks
        total_underlying_shares: u256, // total new shares of underlying contract (e.g. in frmzSTRK)
        user_rewards: LegacyMap<ContractAddress, UserRewardsInfo>, // (user) => UserRewardsInfo
    ...
    > of IRewardShare<ComponentState<TContractState>> {
        ...
        //   let mut current_index = self.n_rewards.read(); // bcz user will get rewards from here

        //   // call init if rewards are not initialized
        //   if (current_index == 0) {
        //     // rewards map is not initialized
        //     self.init(block_number);
        //     current_index = self.n_rewards.read();
        //   }

        //   self.last_updated_block.write(block_number);
        // }

        // fn set_user_rewards_info(ref self: ComponentState<TContractState>, user: ContractAddress, info: UserRewardsInfo, total_shares: u256) {
        //   let owner = self.Ownable_owner.read();
        //   let caller = get_caller_address();
        //   assert(owner == caller, 'Only owner');
        //   let current_index = self.n_rewards.read(); // bcz user will get rewards from here
        //   assert(info.index == current_index, 'Invalid index');
        //   self.user_rewards.write(user, info);
        // }

        /// shares that are not yet created in contract global storage (e.g. in totalSupply var)
        fn get_total_unminted_shares(self: @ComponentState<TContractState>) -> u256 {
        ...
        fn get_additional_shares(
            ...
            let total_rounds = self.n_rewards.read();
            let user_rewards: UserRewardsInfo = self.user_rewards.read(user);
            let mut current_index = user_rewards.index; // computation of points begin here

            // variables updated in loop
            let mut net_additional_shares: u256 = 0;
            ...
            loop {
                ...
                let rewards: RewardsInfo = self.rewards.read(current_index);
                ...
                let block_diff: u256 = (end_block_number - last_block_number).into();
                ...
                let additional_shares = safe_decimal_math::div_round_down(
                    user_round_points * rewards.shares, total_round_points
                );
                /// println!("get_additional_shares: additional_shares: {:?}", additional_shares);

                net_additional_shares += additional_shares;
                user_shares +=
                    additional_shares; // updated total shares will be used for next round
                current_index += 1;
                last_block_number = end_block_number;
                pending_points = 0;
    ...
    > of InternalTrait<TContractState> {
        ...
        fn update_harvesting_rewards(
            ...
            amount: u256, // e.g. USDC reward
            shares: u256, // in lp token terms of the contract (e.g. frmzUSDC) that corresponds to amount
            total_shares: u256,
        ) {
            let mut len = self.n_rewards.read();
            assert(len > 0, 'Rewards not initialized');
            assert(amount > 0, 'Invalid amount');
            assert(shares > 0, 'Invalid shares [1]');

            // generally rewards are always less then total TVL
            // unless everyone withdraws just before harvest.
            assert(shares < total_shares, 'Invalid shares [3]');

            let now = get_block_number();
            let total_round_points = self.get_total_round_points(total_shares);
            let existing_reward_info: RewardsInfo = self.rewards.read(len);
        ...
        fn update_user_rewards(
            ref self: ComponentState<TContractState>,
            user: ContractAddress,
            shares_owned: u256, // current lp shares
            pending_shares: u256, // pending lp shares
            pending_shares_last_block: u64,
            pending_round_points: u256, // pending shares of user in current round
            total_shares: u256, // total lp shares (prev) (e.g. totalSupply frmzSTRK)
        ) {
            ...
            let mut rewards: RewardsInfo = self.rewards.read(current_index);
    ...
    > of PrivateTrait<TContractState> {
        ...
        fn init(
            ...
            let len = self.n_rewards.read();
            assert(len == 0, 'Already initialized');

            self.n_rewards.write(1);
            self.last_updated_block.write(block_number);

            // init current round
            // this will be updated by update_user_rewards when user deposits or withdraws for current round
            self.init_new_round_at_current_index(block_number);
        }

        /// total points accrued since last update
        fn get_total_round_points(
            self: @ComponentState<TContractState>, total_shares: u256,
        ) -> u256 {
            let last_block = self.last_updated_block.read();
            assert(last_block > 0, 'Invalid block number');
            let now = get_block_number();
            let block_diff: u256 = (now - last_block).into();
