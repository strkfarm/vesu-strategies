...
use strkfarm_vesu::interfaces::oracle::{IPriceOracle, IPriceOracleDispatcher, IPriceOracleDispatcherTrait};
...
pub impl AvnuMultiRouteSwapImpl of AvnuMultiRouteSwapTrait {
    fn swap(self: AvnuMultiRouteSwap, oracle: IPriceOracleDispatcher) -> u256 {
        let amount_out = avnuSwap(SwapInfoMinusAmount {
...
pub fn assert_max_slippage(
    ...
    let from_usd = safe_decimal_math::mul_decimals(
        from_price.into(), token_from_amount, 
        from_decimals
    );
    ...
    let to_usd = safe_decimal_math::mul_decimals(
        to_price.into(), token_to_amount, 
        to_decimals
    );

    // max slippage
    let max_slippage = safe_decimal_math::mul_decimals(
        from_usd, max_slippage_bps.into(), 4
    );
